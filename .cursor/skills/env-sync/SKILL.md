...
description: >-
  ...
  in Agent chat for auth-service.
disable-model-invocation: true
---

# Env Sync (auth-service)
...
This skill applies to **auth-service** only:
...
Both files live at the auth-service repo root.
...
PORT=8080               PORT=8080        → skip
DATABASE_URL=postgres…  DATABASE_URL=postgres://custom…  → skip (dev value kept)
JWT_PRIVATE_KEY=        JWT_PRIVATE_KEY=<long key>       → skip (key exists)
```

**Key missing — append from example:**

```
# .env.example has TRUST_PROXY=0
# .env.development has no TRUST_PROXY line
→ append comment block + TRUST_PROXY=0 from .env.example
```
...
1. **Read both files fully** before editing — multi-line JWT keys require it.
